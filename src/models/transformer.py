...
from .weight_init import constant_init_, kaiming_init_, trunc_normal_
...
class MultiheadAttentionWithPreNorm(nn.Module):
    ...
    def __init__(
        ...
        proj_drop=0.0,
        norm_layer=nn.LayerNorm,
        layer_drop=dict(type=DropPath, dropout_p=0.0),
        batch_first=False,
        **kwargs,
    ):
        super().__init__()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for _ in range(num_layers - 1):
            layers.append(
                nn.Sequential(nn.Linear(in_channels, hidden_channels), act_layer(), nn.Dropout(dropout_p))
            )
