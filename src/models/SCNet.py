...
class TIM_Module(nn.Module):
    def __init__(self, in_channels, reduction=16, n_segment=8, return_attn=False):
        ...
        self.htie_conv1 = nn.Sequential(
            nn.Conv2d(
                self.reduced_channels, self.reduced_channels, kernel_size=(3, 1), padding=(1, 0), bias=False
            ),
            nn.BatchNorm2d(self.reduced_channels),
        )
        self.vtie_conv1 = nn.Sequential(
            nn.Conv2d(
                self.reduced_channels, self.reduced_channels, kernel_size=(1, 3), padding=(0, 1), bias=False
            ),
            nn.BatchNorm2d(self.reduced_channels),
        )
    ...
    def feat_ht(self, feat):
        ...
        # [nw, C, h, t]
        feat_ht_out = self.ht_up_conv(
            1 / 3.0 * diff_feat_fwd + 1 / 3.0 * diff_feat_fwd1 + 1 / 3.0 * diff_feat_fwd2
        )
...
class SCConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # sigmoid(identity + k2)
        out = torch.sigmoid(torch.add(identity, F.interpolate(self.f_h(self.f_w(x)), identity.size()[2:])))
...
class SCBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv3 = nn.Conv2d(group_width * 2, planes * self.expansion, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        out_b_1 = self.relu(self.bn1_b(self.conv1_b(x)))

        # spatial representations
        out_b_2, s2t_info = self.scconv(out_b_1)
        out_b_3 = self.relu(out_b_2)
        ...
        out_a = self.relu(
            self.k1(self.shift(self.tim(self.relu(self.bn1_a(self.conv1_a(x)))) + self.ism(s2t_info)))
        )

        if self.avd:
            out_a = self.avd_layer(out_a)
            out_b_3 = self.avd_layer(out_b_3)

        out = self.bn3(self.conv3(torch.cat([out_a, out_b_3], dim=1)))
