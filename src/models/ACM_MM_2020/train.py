...
parser.add_argument(
    "--final_dim", default=1024, type=int, help="length of vector output from audio/video subnetwork"
)
...
parser.add_argument("--img_dim", default=100, type=int)
...
parser.add_argument(
    "--hyper_param", default=0.99, type=float, help="margin hyper parameter used in loss equation"
)
...
def main():
    ...
    model = Audio_RNN(
        img_dim=args.img_dim, network=args.net, num_layers_in_fc_layers=args.final_dim, dropout=args.dropout
    )
...
def test(data_loader, model):
    ...
    model.eval()
    with torch.no_grad():
        for idx, (video_seq, audio_seq, target, audiopath) in tqdm(
    ...
    write_log(
        content="Loss {loss.avg:.4f}\t".format(loss=losses, args=args),
        epoch=num_epoch,
...
def set_path(args):
    if args.resume:
        ...
        exp_path = "log_tmp/deepfake_audio-{args.img_dim}_{0}_bs{args.batch_size}_lr{1}".format(
            "r%s" % args.net[6::], args.old_lr if args.old_lr is not None else args.lr, args=args
        )
