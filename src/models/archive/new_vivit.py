...
from .weight_init import (
    ...
    init_from_mae_pretrain_,
    init_from_vit_pretrain_,
    trunc_normal_,
)
...
class ViViT(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mlp_head = nn.Linear(embed_dims, 2)
...
class MaskFeat(nn.Module):
    ...
    def forward(self, x, target_x, mask, cube_marker, visualize=False):
        ...
        for i, mark_item in enumerate(cube_marker):  # loop for batch
            for marker in mark_item:  # loop for video sample
                ...
                center_index[start_frame * self.stride[0] + span_frame * self.stride[0] // 2] = (
                    1  # center index extends to 16
                )
        ...
        if visualize:
            ...
            mask_preds = rearrange(
                mask_preds, "b t h w (dh dw c o) -> b t (h dh) (w dw) c o", dh=2, dw=2, c=3, o=9
            )  # need to unnormalize
    ...
    def forward_features(self, x, mask=None):
        ...
        if mask is not None:
            ...
            dense_mask = repeat(
                mask, "b t h w -> b t (h dh) (w dw)", dh=self.downsample_rate, dw=self.downsample_rate
...
def parse_args():
    ...
    # Model
    parser.add_argument(
        "-arch", type=str, default="mvit", help="the choosen model arch from [timesformer, vivit]"
    )
    # Training/Optimization parameters
    parser.add_argument("-optim_type", type=str, default="adamw", help="the optimizer using in the training")
    parser.add_argument("-lr", type=float, default=0.0005, help="the initial learning rate")
    parser.add_argument("-layer_decay", type=float, default=1, help="the value of layer_decay")
    parser.add_argument(
        "-weight_decay",
        type=float,
        default=0.05,
