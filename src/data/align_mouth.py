...
def load_args(default_config=None):
    ...
    parser.add_argument(
        "--window-margin", default=12, type=int, help="window margin for smoothed_landmarks"
    )
...
def crop_patch(
    ...
    while True:
        ...
        if len(q_frame) == margin:
            ...
            sequence.append(lip_img)
            lip_file = os.path.join(output_path, f"lip_{lip_idx}.png")
            cv2.imwrite(lip_file, lip_img)
            lip_idx += 1
...
if __name__ == "__main__":
    ...
    for filename_idx, filename in enumerate(tqdm(fids)):
        ...
        assert os.path.isfile(landmarks_pathname), "File does not exist. Path input: {}".format(
            landmarks_pathname
        )
