...
# from lightning.utils.types import T_LABEL
...
class Fakeavceleb(Dataset):
    ...
    def __init__(self, subset: str, root: str = "data", metadata: Optional[List[Metadata]] = None):
        self.subset = subset
        self.root = root
...
class FakeavcelebDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        self.metadata = pd.read_csv(os.path.join(self.root, "meta_data.csv"))  # .loc
        ...
        train_metadata_B = train_metadata[train_metadata["category"].isin(["B"])][:train_count]
        train_metadata_C = train_metadata[train_metadata["category"].isin(["C"])][:train_count]
        train_metadata_D = train_metadata[train_metadata["category"].isin(["D"])][:train_count]
        ...
        #
        test_metadata_A = test_metadata[test_metadata["category"].isin(["A"])][:test_count]
        test_metadata_B = test_metadata[test_metadata["category"].isin(["B"])][:test_count]
        test_metadata_C = test_metadata[test_metadata["category"].isin(["C"])][:test_count]
        test_metadata_D = test_metadata[test_metadata["category"].isin(["D"])][:test_count]
        #
