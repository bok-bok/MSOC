...
class DatasetCreater:
    ...
    def get_fake_data(self, data: list):
        ...
        def check_valid(video_row, audio_row):
            if (
                video_row["subject1"] != audio_row["subject1"]
                or video_row["subject2"] != audio_row["subject2"]
            ):
                ...
                # visual
                    ...
                    for filename in filenames:
                        (
                            ...
                            model_name,
                            blending_name,
                        ) = self.get_visual_modal_info(filename)
                        # id = subject + "-" + subject2
                        file_dir = os.path.join(root, filename)
                        visual_data.append(
                            {
    ...
    def get_original_data(self, data):
        ...
        def update_data_with_dirs(dirs: list, data: list, group: str):
            for dir in dirs:
                data.append({"video_dir": dir, "group": group, "target": ORIGINAL})

        def check_include_audio(video_name: str):
            # video id ends with 2 has audio
            if video_name[-1] == "2":
                return True
            else:
                return False
        ...
        for root, _, filenames in os.walk(ORIGINAL_SWAN_DIR):
            for filename in filenames:
                ...
                if check_include_audio(video_name):
